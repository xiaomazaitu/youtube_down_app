...
# 或者上传项目文件到服务器
# scp -r /本地后端项目路径/* user@服务器IP:/opt/youtube_app/
```
...
# 或者上传前端文件到服务器
# scp -r /本地前端项目路径/* user@服务器IP:/opt/youtube_web/
...
# 创建后端视频存储目录
mkdir -p /opt/youtube_app/cache/videos
...
### 4.2 更新后端应用配置
```bash
# 编辑后端config/celery_config.py，确保配置正确
...
```bash
# 创建定期清理脚本
cat > /opt/youtube_app/cleanup.sh << 'EOF'
#!/bin/bash
# 清理过期日志文件
...
# 清理过期缓存文件（如果应用清理不及时）
find /opt/youtube_app/cache -name "*" -mtime +1 -delete
EOF

chmod +x /opt/youtube_app/cleanup.sh

# 添加到crontab
crontab -e
# 添加以下行：
# 0 2 * * * /opt/youtube_app/cleanup.sh
```
...
```bash
# 创建备份脚本
cat > /opt/youtube_app/backup.sh << 'EOF'
#!/bin/bash
...
# 备份后端应用代码
tar -czf $BACKUP_DIR/app_$DATE.tar.gz /opt/youtube_app

# 备份前端代码
tar -czf $BACKUP_DIR/web_$DATE.tar.gz /opt/youtube_web
...
chmod +x /opt/youtube_app/backup.sh
```
